# LOGICALDEDUCTION with DetermLR
import requests
import guidance
import ast
import argparse
...
def get_parser():
    parser = argparse.ArgumentParser(description="Deterministic LR")
    parser.add_argument('--temperature', type=float, default=0.1, help='temperature')
    parser.add_argument('--propnum', type=int, choices=range(0, 21), default=3, help='numbers of props')
    ...
    parser.add_argument('--condition_divide', default=True, help='determinate or not')
    return parser
...
def main():
    ...
    # Load the data from the JSON file no need extra translation
    ...
    logfilename = 'results/logicdeduction/results-determlr-openai--' + model_name + '--t' + str(
    ...
    total_cnt = len(data)
    total_nodes = 0 
    # Iterate over the data from the JSON file and call the DetermLR function
    for example in tqdm(data, desc="Evaluating", unit="example"):
        ...
        deter_num = 0
        indeter_num = 0
        # while (deter_num<args.propnum and indeter_num<args.propnum and failed_cnt < args.trycnt):  
        # determinate and indeterminate both judge the loop
        while (deter_num < args.propnum and failed_cnt < args.trycnt):
            visited_nodes += 1
            failed_cnt += 1
            explanation = ""
            if args.verbose: print("\t# <No. {}>".format(len(new_premises) + 1))

            if args.con_select:
                if failed_cnt >= (args.trycnt / 2):  
                    # random choose when fail times reach the threshold
                    if numpy.random.rand() < args.exploration_prob: 
                        tmp = numpy.random.choice(premises + new_premises,
                                                  size=min(len(premises + new_premises), args.choices), replace=False)
                    else:
                ...
                if failed_cnt < (args.trycnt / 2):
                    try_cnt = 0
                    while try_cnt < TRY_CNT:  # 根据这组条件生成结论
                        try:
            ...
            if failed_cnt < (args.trycnt / 2) and args.con_select:
                clauses = tmp.split('.')
            ...
            # is something validation
            is_something_selection = 'False'  
            # new premise with the following format is invalid (Experimental experience)
            ...
            while try_cnt < TRY_CNT:
                try:
                    sourced_local = \
                    sourced_deduction(examples=sourced_deduction_examples, premises=' '.join(tmp), proposition=prop, boundary_condition=boundary_condition,
            ...
            #  Logical validation: Check existing premises, which may conflict with unselected premises.
            try_cnt = 0
            while try_cnt < TRY_CNT:
                try:
                    validation_local = \
                    validate_deduction(examples=validate_deduction_examples, premises=' '.join(tmp), proposition=prop, boundary_condition=boundary_condition,
            ...
            while try_cnt < TRY_CNT:
                try:
                    validation_global = \
                    validate_deduction(examples=validate_deduction_examples, premises=' '.join(premises + new_premises), boundary_condition=boundary_condition,
                ...
                try:
                    boundary_judgement = \
                        boundary_deduction(examples=boundary_deduction_examples, boundary_condition=boundary_condition,
                                           premises=' '.join(premises + new_premises),
            ...
            # useful deduction and new premise identification when setting is true.
            ...
            new_premises.append(prop) 
            # reasoning structure memory 
            infer_history.append(
                "In the NO:{} round,".format(len(new_premises)) + " we use these \"premises\": \"" + ' '.join(
                    my_tmp) + "\"" + "and got a \"New Determinate Premise\": \"" + prop + "\"\n")
            Last_infer_history = ['There is no information, because this is the first reasoning.']

            failed_cnt = 0

        if args.verbose: print("[Generated new_premises]: \t", new_premises)
                ...
                try:
                    ...
                    if args.memory:
                        out = structure_program_memory(
                            ...
                            boundary_condition=boundary_condition,
                            new_premises=' '.join(new_premises),
                            infer_history=infer_history,
                            choose=choose,
                            temperature=t,
                        )
                ...
                while try_cnt < TRY_CNT:
                    try:
                        verified_reasoning = validate_deduction(examples=validate_deduction_examples,
                                                                premises=' '.join(premises + new_premises), boundary_condition=boundary_condition,
        ...
        result = {
            ...
            "determinate_premise:": ' '.join(determinate_premise),
            "indeterminate_premise:": ' '.join(indeterminate_premise),
            "generated_new_premises": new_premises,
