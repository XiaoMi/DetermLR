...
def main():
    ...
    for example in tqdm(data, desc="Evaluating", unit="example"):
        ...
        deter_num = 0
        indeter_num = 0
        while (len(propositions) < args.propnum and failed_cnt < args.trycnt ):  # number of propositions
            failed_cnt += 1
            ...
            if args.con_select:
                ...
                if args.tot:
                    ...
                    for i in range(0, args.bfs):
                        # randomly choose a set of premises
            ...
            # For bfs, when visit a state, we put a numebr of 'bfs' propositions into queue
            if args.tot:
                for i in range(0, args.bfs):
                    ...
                    # if prop begin with 'There is no' or 'No valid' or 'None of the' or with 'new proposition', then skip
                    ...
                    while try_cnt < TRY_CNT:
                        try:
                            sourced_local = \
                            sourced_deduction(examples=sourced_deduction_examples, premises=' '.join(tmp[i]), proposition=prop,
                    ...
                    if validation_local == 'False':
                        Last_infer_history = "In the last round,we use this \"most relevant premise\": \"" + last_relevant_premise + "\"" + "and got a \"false Proposition\": \"" + prop + "\""
                        continue
                    # global validation test whether there is a conflict in other premises 
