...
def get_parser():
    ...
    parser.add_argument('--dataset', type=str, default='data/proofwriter/proofwriter-test.json', help='dataset to use')
...
def main():
    ...
    logfilename = 'results/proofwriter/results-determlr-openai--' + model_name + '--t' + str(
    ...
    for example in tqdm(data, desc="Evaluating", unit="example"):
        ...
        # memory use and premise identification
        ...
        while (deter_num < args.propnum and indeter_num < args.propnum and failed_cnt < args.trycnt):  # number of determinate premises
            ...
            if args.con_select:
                if failed_cnt >= (args.trycnt / 2):
                    if numpy.random.rand() < args.exploration_prob:  # when fail times reach the threshold
                ...
                if failed_cnt < (args.trycnt / 2):
                    try_cnt = 0
                    while try_cnt < TRY_CNT:  # base on this set of premises
                        try:
                ...
                if args.tot:
                    for i in range(0, args.bfs):
                        ...
                        if numpy.random.rand() < args.exploration_prob:  
                            tmp.append(numpy.random.choice(premises + new_premises,
                                                      size=min(len(premises + new_premises), args.choices),
                                                      replace=False))
                        else:
            ...
            # invalid premise with the following format
            ...
            if args.global_validation:
                ...
                while try_cnt < TRY_CNT:
                    try:
                        validation_global = \
                        validate_deduction(examples=validate_deduction_examples, premises=' '.join(premises + new_premises),
            ...
            # useful deduction and premise identification 
