...
def get_parser():
    ...
    parser.add_argument('--trycnt', type=int, choices=range(1, 1001), default=8, help='numbers of try times')
    ...
    parser.add_argument('--boundary_validation', default=True, help='use boundary or not')
    ...
    parser.add_argument('--infer_history', default=False, help='use validation global or not')
    return parser
    ...
    for example in tqdm(data, desc="Evaluating", unit="example"):
        ...
        while (deter_num<args.propnum and indeter_num<args.propnum and failed_cnt < args.trycnt):  # number of determinate premises
            ...
            if args.con_select:
                ...
                if failed_cnt < (args.trycnt / 2):
                    try_cnt = 0
                    while try_cnt < TRY_CNT:  # base on these premises
                        try:
                ...
                if numpy.random.rand() < args.exploration_prob:  # randomly choose a set of premises
                    tmp = numpy.random.choice(premises + propositions,
                                              size=min(len(premises + propositions), args.choices), replace=False)
                else:
                    tmp = numpy.random.choice(premises, size=min(len(premises), args.choices), replace=False)
            ...
            try_cnt = 0
            while try_cnt < TRY_CNT:  # exploration based on these premises
                try:
            ...
            # this is a format problem from guidance
            if len(prop) > 0 and prop[-1] == '"':
                prop = prop[:-1]

            # When duplicate props are generated, this round is skipped.
            ...
            # useful validation when setting is true.
        ...
        reasoning_num = 0
        reasoning_try_cnt = 0
        judgement_cnt = {"A": 0, "B": 0, "C": 0, "D": 0, "K": 0}   # Option "k" represents the situation where the answer cannot be selected
        reasoning_list = []
